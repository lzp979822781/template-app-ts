...
const utils = {
  ...
  redirectToH5({ page, wvroute }) {
    const url = plugin.formH5Url({ page: decodeURIComponent(page), wvroute })
  ...
  */
  ...
  handleJump(p = {}) {
    const { goback, pluginUrl, riskUrl } = p;
    if (goback) {
      utils.goBack();
      return
    }
  ...
  h5Init(options) {
    const p = plugin.getLoginParams();
    if (plugin.isEmptyObj(p)) utils.setLoginParamsStorage(options)
  },
  setCustomNavigation() {
    const { navigationBarColor, navigationBarTitle } = plugin.getLoginParams();
